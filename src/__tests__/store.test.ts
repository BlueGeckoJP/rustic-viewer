...
describe("tab store", () => {
  ...
  test("addSingleTab adds a tab and sets activeTabId", () => {
    ...
    expect(s.tabs[id]).toBeDefined();
    expect(s.activeTabId).toBe(id);
    expect(isSingleTab(s.tabs[id])).toBe(true);
  });
  ...
  test("addComparisonTab creates comparison tab with children", () => {
    ...
    const tab = useTabStore.getState().tabs[id] as any;
  ...
  test("createComparisonFromSingleIds moves selected singles into comparison", () => {
    ...
    const tab = stateAfter.tabs[active!];
    expect(tab).toBeDefined();
    expect(tab.type).toBe("comparison");
    // moved singles should not exist as top-level tabs
    expect(stateAfter.tabs[a]).toBeUndefined();
    expect(stateAfter.tabs[b]).toBeUndefined();
  });
  ...
  test("setCurrentIndex updates single and child indices", () => {
    ...
    expect(
      useTabStore.getState().tabs[id] &&
        (useTabStore.getState().tabs[id] as any).currentIndex
    ).toBe(1);
    ...
    expect(comp.children[child.id].currentIndex).toBe(1);
  });
  ...
  test("detachChildToTopLevel and detachAllChildren behave correctly", () => {
    ...
    expect(useTabStore.getState().tabs[c1.id]).toBeDefined();
    ...
    expect(useTabStore.getState().tabs[newCompId]).toBeUndefined();
  });
});
